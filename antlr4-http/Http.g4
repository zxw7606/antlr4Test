 ...
 */
http_messages: http_message+ EOF;

http_message: (CRLF* comment CRLF)? start_line (header_field CRLF)* CRLF (message_body)? //message_body
            ;
 ...
 */
field_vchar: vCHAR | obs_text | pchar | SP;
/*
 obs-text= %x80-FF
 */
obs_text: OBS_TEXT;
 ...
 */
urlchar: unreserved | Pct_encoded | sub_delims | Colon | At | Slash | CHINESE;
pchar: unreserved | Pct_encoded | sub_delims | Colon | At | Slash | QuestionMark | CHINESE;
...
message_body: (pchar|vCHAR|CRLF|SP)*;
comment: Hashtag Hashtag Hashtag SP (pchar|vCHAR)*;
